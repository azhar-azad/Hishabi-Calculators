# Calculators — Progress Archive (Phases 0–3)
...
- [x] Commit `feat(tax): GET /api/calculators/tax/rules/{year}`; push — committed as `700c2bb` (other machine/session)

### 3.15 — Dev seeding (Flyway on H2 dev)
- [x] Run Flyway on the H2 dev profile so a locally-run backend has the seeded rule set (was a gap: Flyway was off on H2, so dev had tables via Hibernate `create-drop` but **no seed data** — calculate would 404 "No assessment years configured"). **Fix:** `application-dev.yml` now sets `spring.flyway.enabled: true` and `ddl-auto: create-drop` → **`validate`** (Flyway owns the schema; `create-drop` would have dropped Flyway's tables and wiped the seed). Verified the migrations apply on H2 2.4 in PostgreSQL mode (incl. `GENERATED BY DEFAULT AS IDENTITY` and `ALTER COLUMN ... RESTART WITH`). **Supersedes the slice-3.3 "Flyway off on H2" stance** — dev now mirrors prod's data path (same migrations + seed, `validate` everywhere). Single source of truth, no separate dev seed file → zero drift. Recorded in PLAN.md §2
- [x] Test: `DevProfileBootTest.devProfileIsSeededByFlyway` asserts `findByLabel("2025-26")` is present in the dev context. `ProdProfileBootTest` (explicit `flyway.enabled=false` + H2) and the Testcontainers tests (explicit PG) are unaffected. 62/62 backend tests green via `./mvnw verify`
- [x] Self code-review (medium) — config + test only; no findings. dev/prod data paths now unified
- [x] Commit `fix(backend): seed dev (H2) via Flyway`; push
